...
# Target the switch in _refreshTabInternal_ which has Bank Processing
# We'll use regex to inject it before "case 'Bank Processing':" but ONLY in _refreshTabInternal_

# Let's split by function to be safe
refresh_fn_start = relay.find("function _refreshTabInternal_")
save_fn_start = relay.find("function saveTab_")

refresh_part = relay[refresh_fn_start:save_fn_start]
refresh_part = refresh_part.replace("    case 'Bank Processing':", refresh_missing + "\n    case 'Bank Processing':")

relay = relay[:refresh_fn_start] + refresh_part + relay[save_fn_start:]
