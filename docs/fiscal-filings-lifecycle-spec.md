...
// periods.tax_attrs, per fiscal-year period row
{
  ...
  "ar_facts": {                                // Annual Report governance facts — record-keeping only, no renderer consumes this after §8
...
| `filing.set_due_override` | `{ key, dueDate }` | `dueDate: null` clears. Read/patch/write the `deadline_overrides` settings row (same per-key upsert pattern `settings.save` already uses, `api/src/index.js` lines 1800–1813) — a dedicated row per company, so this is safe from the `tax_attrs`-style whole-column-clobber risk without needing the new helper. |
| `filing.save_period_attrs` | `{ periodId, patch: { loss_cf?, periodiseringsfond?, ar_facts? } }` | Via `patchPeriodTaxAttrs`: `loss_cf` replaced wholesale if provided; `periodiseringsfond` replaced wholesale (full array) if provided; `ar_facts` shallow-merged key-by-key if provided (partial edits don't clobber other facts). Write path for manual loss-carryforward correction, periodiseringsfond tranche edits, and the Annual Report facts form. |
...
Part of `filing.mark_submitted` when `key` is `ink2@...` and `method` succeeds: scan `computed.descriptor.fields` (the loaded `ink2.json`) for the entry with `op === 'loss_closing'`, read the computed value at `computed.fields[thatBlankett][thatCode]` — this is the year's closing loss. **Only if next fiscal year's period row already exists** (never auto-create a period), `patchPeriodTaxAttrs` that next period to set `loss_cf` to this value. If the next period doesn't exist yet, skip silently — the figure carries once that period is created and this year's filing is (re-)marked submitted, or via the manual `filing.save_period_attrs` override.
...
- New inline expand-rows (chip toggles a `<tr>` below — a fresh, small version of the idea behind `payables.js`'s `.preview-row`, since `fiscal.js` has no such row type yet):
