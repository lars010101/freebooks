...
- ~~Bills due/overdue for payment (posted payables awaiting settlement — **not** drafts; bill drafts are Class A above, per the Option C amendment)~~ **Moved 2026-09-05** to the notifications bell (§10.7 item 4 amendment below) — it never carried an in-place decision, only a navigate-away verb, which is what the bell is for.
...
**Superseded 2026-09-05 — bill_due and reconciliation_alert moved out of the Inbox to the notifications bell.** Neither item type carried an in-place decision — the `o` verb above only ever navigated away (to Payables, or to the Bank Reconciliation tab for reconciliation_alert) — so they belong with the bell's other "go look at this" alerts (fx-gap, reminder-due), not this decide-here approval queue. `status='bills'`/`status='reconciliation'` were removed from `inbox.js`'s `listInbox` dispatch entirely; `queryBillsDue`/`queryReconciliationAlerts` moved to new `bills-due-scanner.js`/`reconciliation-scanner.js` (daily interval, same shape as `reminder-scanner.js`), which call `raiseNotification` with a new `link_url` column — bill-due links straight to `/:company/bill/:id` (more precise than the old `o` verb's bare `/payables`), reconciliation-alert to `/:company/bank?tab=reconciliation` (unchanged target). The Inbox `f` cycle is now four states: proposed → rejected → orphans → partners — exactly the two Class B types (`orphan_file`, `partner_proposal`) that still have a real in-place verb (delete; approve/reject). §10.2's Class B taxonomy list below is written as of 2026-08-05 and predates this change; `bill_due`/`reconciliation_alert` are no longer Inbox item types.
