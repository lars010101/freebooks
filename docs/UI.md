...
- **A control never implies capability the backend doesn't have.** An interactive-looking element (a dropdown offering choices, a button suggesting an action) that silently does nothing different no matter which option is picked is worse than showing the same information as a plain read-only badge — it teaches the reviewer a false model of what the app can do. **Shipped 2026-09-09** (Inbox's Partners tab Duplicate column): the finalized mockup specified a "create as alias of X" dropdown, but no alias/duplicate-merge concept exists anywhere in the backend (`findFuzzyMatch` only ever returns one best-guess candidate, and there's no merge action at all) — built as the existing informational warning badge instead of a dropdown that would look actionable without being able to actually act.
