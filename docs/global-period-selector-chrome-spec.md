...
**Date:** 2026-08-27 · **Status:** Built — implemented in `api/public/fb-core.js` (`FB.period` module, popover) and `api/src/pages/common.js` (bar swap). Status corrected 2026-09-01; the spec had shipped without its header being updated off PROPOSED.
...
Renders in the top bar, immediately right of the company name (§2.1). Collapsed state shows the current resolved label (a period name, or `start – end` in Custom mode) — **blank, not a "Period" placeholder, when nothing has resolved** (no periods configured for the company at all; corrected 2026-09-03, Magnus — a permanent "Period" placeholder reads as a stuck control, not an empty one). The trigger stays clickable when blank: `.tb-period-trigger` carries an explicit `min-width`/`min-height` for exactly this state, since a bare empty `<span>` collapses to zero height and disappears — which would silently violate the dimmed-never-hidden doctrine (§0 item 3) for the one state that needs it most (a brand-new company has to be able to click into the control to discover it has no periods yet). Click opens a popover — model its positioning on the same `position:fixed; top:52px` pattern used for `.tb-notif-dropdown` and the relocated company dropdown (§2.1), not a third positioning strategy.
...
**Whole-popover Enter/Escape (ratified 2026-09-01, magnus):** while the popover is open, `Enter` commits (equivalent to the "close on select" behavior above — if Custom mode has a pending, uncommitted date edit, it's committed first) and closes as OK; `Escape` discards anything changed since the popover opened — reverting to a snapshot of `FB.period.get()` taken at open time via the existing `set()` (§3.7), which does not persist to localStorage — and closes as abort. This is standard modal Enter/Escape semantics (keyboard-ux-spec.md §7's confirm/cancel contract), not a new letter-verb, applied to a control that previously only closed via outside-click or a Period pick.
...
1. **Explicit navigation intent** — a `?period=` or `?start=`/`?end=` URL param (drill-through from a report, a return-context link, or historically a `:report` command-bar target before `:` was retired). Always wins; this is a deliberate override, not a default.
...
## 8. Keyboard interaction — `p`/`n` quickset (added 2026-09-03)

**`p`/`P` freed app-wide 2026-09-02.** `bill-post-payment-consolidation-spec.md` retired both from Bills/bill-edit that date (superseded by `w`/`y`); bank-import's own `p` = paste-CSV had already died with that page's deletion (issue #137, 2026-08-09). Grepping the app confirmed both keys fully unbound before this addition — nothing to reconcile.

**`p` opens this control.** In NORMAL mode, outside any active page-set claim on `p` (context-override doctrine, mirroring the `g`-prefix's own claim check, `keyboard-ux-spec.md` §2), a bare `p` opens the popover exactly as clicking the trigger does — it never toggles closed the way a click does; only Escape, Enter, a row pick, or outside-click close it. `P` (capital) is deliberately left unbound by this addition.

**While open, the popover owns every key not typed into its date inputs** — same "owns every key" doctrine as the company switcher (`keyboard-ux-spec.md` §3), with one carve-out: focus inside the Start/End inputs (§3.2) falls through untouched except for Enter/Escape, so native date-input editing (digit entry, segment arrow-stepping, Tab between segments) is never hijacked.

| Key | Action |
|---|---|
| `j` / `↓` | Highlight next row (sticky at bottom) |
| `k` / `↑` | Highlight previous row (sticky at top) |
| `Enter` | Apply the highlighted period and close; if Custom is highlighted, commit the typed Start/End and close (unchanged from the existing whole-popover Enter contract, §3.2) |
| `p` | Quickset: apply the previous period/range and close |
| `n` | Quickset: apply the next period/range and close |
| `Esc` | Close, reverting to the pre-open snapshot — unchanged, §3.2 |

**`p`/`n` act on the currently ACTIVE period or range, never on the popover's highlight** — moving the highlight with `j`/`k` to preview a different row does not change what `p`/`n` step from.

- **Configured period active:** steps to the adjacent entry in the periods list (§3.4's descending `start_date` sort). At either end of the list, `p`/`n` is a no-op — nothing to step to, popover stays open.
- **Custom range active:** only defined when the active range is an *exact* calendar unit — Jan 1–Dec 31 (year), first-to-last day of one month (month), a Monday–Sunday week, or a single day. The step moves by that unit. An arbitrary custom range (anything not matching one of those four exactly) makes `p`/`n` a no-op. A same-length day-shift fallback for arbitrary ranges was considered and rejected: a silent, unexplained date shift on a range the user typed by hand is more surprising than nothing happening.

**`n` has no meaning outside this interaction** — it is not a global binding; pressing it while the popover is closed does nothing. Stepping back one period therefore reads as two keystrokes (`p p`: open, then quickset-previous), not one — there is no single-key shortcut that bypasses the open step. This needs no artificial delay: the popover opens synchronously and unconditionally on the first `p` (it is never ambiguous with what follows), so the second keystroke is never dropped waiting to disambiguate a chord.

**Popover row order corrected to match §3.2.** §3.2's own diagram always specified defined periods first, "Custom" last — the original `<select>`-based implementation had Custom first, a drift from spec caught while building this addition. The popover was rebuilt as `.tb-period-row` divs (mirroring `.tb-company-opt`) instead of a native `<select>`, which is what makes the `j`/`k`/`p`/`n` keyboard contract above possible — a native select's own arrow-key handling would have fought with it. `"all-time"` (no date bound at all) was considered as a `p`-adjacent quickset and explicitly rejected for this addition: no such concept exists anywhere else in the app today, `FB.period` is architected to always resolve to a concrete range, and Bills/FX Rates specifically guard against an unscoped load as a hazard (`fb-list-default-period-spec.md`) — adding it would need per-screen opt-in, not a blanket global key.

**Start/End (or As-of) date fields corrected to actually match §3.2 — a second implementation drift caught in review, not a new decision.** §3.2 already specified "when Custom is selected: [Start]/[End]" as the SECOND step, distinct from picking a period; the `<select>`-era implementation had rendered the date fields unconditionally underneath the dropdown regardless of mode, which visually sat between the list and made the list unreachable below that point by keyboard scrolling, and defeated the "Custom is a second step" model entirely (caught by Magnus testing the rebuilt popover, 2026-09-03). Fixed: `_pBuildPopover` now renders the date-fields block only when `_pState.mode === 'custom'`; picking the Custom row (mouse or keyboard) rebuilds the popover to reveal them and focuses Start. Two further bugs surfaced fixing this, both fixed in the same pass: (1) the Custom row's `onclick` rebuilding the popover's `innerHTML` mid-click detached the very row the click was bubbling from, which made the document-level outside-click-closes listener see a "click outside" and close the popover the same frame it was told to stay open — fixed with `event.stopPropagation()` on the row click, same pattern `togglePopover` already used for its own trigger; (2) keyboard `Enter` on a highlighted Custom row that hadn't been entered yet (mode still a defined period) fell into the "already custom, commit and close" branch instead of the "first switch, reveal fields, stay open" one — `key('Enter')` now distinguishes the two by checking whether `_pState.mode` is already `'custom'`.
...
| 2026-09-01 | Status header corrected PROPOSED → Built — the control has been live in code since shortly after drafting; the header was simply never updated. §3.2 gained a ratified whole-popover Enter (commit/OK) / Escape (revert-to-snapshot/abort) behavior, added to the implementation this same date. Note: the `:vat-tolerance`/`:gst-tolerance` cutover gate referenced in this session's discussion lives in `global-search-spec.md` §0/§8, not in this document — it has been resolved there, not here; this spec never carried that gate. |
| 2026-09-03 | New §8: `p` freed app-wide (Bills/bank-import retirements, 2026-09-01/02) now opens the Global Period Selector; `p`/`n` quicksets to the previous/next period (or, for an exact calendar month/week/year/day Custom range, that unit) while the popover is open, applying immediately and closing. Popover rebuilt from a native `<select>` to `.tb-period-row` divs (mirroring the company switcher) so `j`/`k`/arrow highlight navigation works without fighting native select behavior; row order corrected to match §3.2's always-Custom-last diagram (the `<select>` build had drifted to Custom-first). `keyboard-ux-spec.md` gained a matching §10. An "all-time" quickset was considered and explicitly rejected — see §8's closing paragraph. |
| 2026-09-03 | Same-day follow-up, caught in testing: the rebuilt popover had also drifted from §3.2 by rendering the Start/End date fields unconditionally instead of only after Custom is picked, which blocked reaching rows below them and defeated the two-step Period-then-Custom model. Fixed along with two bugs the fix surfaced (a click-triggered popover self-close race, and keyboard Enter on a not-yet-entered Custom row closing instead of revealing the fields) — see §8's new closing paragraph for detail. |
| 2026-09-03 | §3.1: collapsed trigger shows blank, not a "Period" placeholder, when no periods are configured (`_pLabel` returned the literal string `'Period'` in that case, permanently). Fixing the label alone regressed the trigger to zero height (an empty `<span>` with no explicit box collapses and disappears) — `.tb-period-trigger` gained `display:inline-block; min-width; min-height` in the same pass so the blank state stays clickable, consistent with §0 item 3's dimmed-never-hidden doctrine. |
