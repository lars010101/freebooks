...
1. **Tab content restructure** (lands with each tab's own migration):
   - **Company tab** → slim **current-company** record only; the all-companies grid moves to `admin.js` (stays as spec'd). **Rev. 2026-07-27:** FX provider config is **install-level** (see `fx-automation-spec.md` rev. 2026-07-27) — provider `<select>` + API key live on the **admin page** behind an explicit **Save** button, not on the Company tab. The slimmed Company record instead gains the per-company **`fx_tracking` checkbox** (`'off'` = domestic-only: simplified UI + no scanning — fx-automation-spec §1). Auto-save-on-select remains abolished (one-save-path doctrine).
   - **Admin page chrome (agreed 2026-07-27):** the admin page does not exist yet (today `admin.js` is only the token-gated SQL endpoint) — it is created as part of this item, reached via a **`🛠 Admin` sidebar entry** at the bottom of the sidebar nav (after Settings) → route `/admin` (install-level, not company-scoped); joins the shared `{`/`}` page rotation like every other sidebar page. **No auth gate for now** (Magnus, 2026-07-27) — it is the first thing to gate when auth lands (holds install-level API keys + destructive company actions). The companies grid runs the FB.list read-first doctrine like every other register. The existing top-left company switcher (click the company name in the sidebar header — `fbToggleCompany`) stays unchanged; no new switch mechanism.
...
5. **Exchange Rates tab — panel moves, Fetch Rates stays.** The FX provider `<select>` + API-key row leave the Exchange Rates tab for an explicit-Save panel elsewhere; auto-save-on-select is abolished (one-save-path doctrine). **Rev. 2026-07-27:** the destination is the **admin page** (install-level config), not the Company tab as originally written — the per-company implementation that shipped 2026-07-23 on the Company tab is superseded when the FX-automation build lands. **Fetch Rates stays on the Exchange Rates tab** as a list-level action (fb-list-ux-spec §8): `f` verb + one small header button (mouse parity); it imports new rows and must not edit existing ones. **2026-07-23 (rev. 2):** the register is **fully editable like every other list** — no read-only ECB rows, no dimming, delete allowed. A user write **flips the row's source to `manual`**: the client sends the original saved key and the server replaces that row (no ecb+manual duplicates). The legacy bulk **Save Rates button is removed** (one save path: `w`).
