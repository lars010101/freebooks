...
3. Server partitions the requested categories against `chat_data_permissions` (§2b):
   ...
   - **Undecided** → fetch now *for local preview only*, and return a `pending_permission` response to the client instead of completing the turn. `chat.send` does not block on human input — a turn with pending categories is parked (`chat_pending_turns`, §2b) and resumed by a separate call once decided.
4. Client renders one inline consent card per pending category, showing the actual preview data (§2c) — never just the category name.
5. User decides, per category, via `chat.permission.decide` (`turnId`, `category`, `decision` ∈ `approve_once | allow_always | deny_once | deny_never`). `allow_always`/`deny_never` write a row to `chat_data_permissions`(persists for future turns); `approve_once`/`deny_once` affect only this turn.
...
9. Both the user's message and the final assistant reply are written to `chat_messages` (§4) once the turn completes; a turn that never resolves its pending permissions leaves no `chat_messages` rows, only a `chat_pending_turns` row.
...
| `journal_entries` | Posted journal lines, filterable by account/date range/description substring (the filter itself is LLM-supplied text/params, validated as an ordinary query filter — not a raw SQL fragment) | Real ledger detail — this is the "just a worse UI" gap named in the design discussion; without it chat can't actually answer ledger questions. |
...
New table `chat_data_permissions`: `company_id, category, decision ('allow_always'|'deny_always'), decided_by, decided_at`. One row per category per company; a fresh company starts with no rows (everything undecided → first use always prompts).

New table `chat_pending_turns`: `company_id, turn_id, user_message, pending_categories (JSON array), resolved_categories (JSON — category → {decision, data-if-approved}), created_at`. Deleted once the turn resolves and Call 2 completes; this is working state, not conversation history.
...
- **The actual data that would be sent** — the real fetched rows (or the aliased version, if aliasing applies to that category per §3 — showing exactly what would leave, in whichever form it would leave in).
- Four buttons: Approve once · Always allow · Deny once · Never allow.
...
Aliasing is a **per-category choice at consent time**, not a separate toggle: the "Approve once / Always allow" decisions in §2b/§2c gain a variant for any category containing `partner_name`/company identity fields — e.g. `journal_entries` and `bills` would each offer "Always allow (aliased)" alongside plain "Always allow". The consent card's data preview (§2c) shows the aliased form when that option is chosen, so the human sees exactly what leaves either way.
...
**On the topbar icon:** a color dot — green (agent running AND LLM reachable), amber (agent running, LLM unreachable — the exact split #180's original research called out as the case worth distinguishing), grey (agent stopped). Computed once per app-shell load and re-checked every 5 minutes while the tab stays open (not on every page navigation — avoids hammering `agent.status`/`ai.test_connection` on every soft-nav).

## 5. Persistence — full table list

- `chat_messages` — `company_id, message_id, role ('user'|'assistant'), content, proposal_ref (nullable), created_by, created_at`. Written only for a completed turn (§2 step 9).
...
- `chat.permission.decide` — role `data_entry`. params: `turnId: {type:'string', required:true}`, `category: {type:'string', required:true}`, `decision: {type:'string', required:true}` (`approve_once|allow_always|deny_once|deny_never`, optionally suffixed `_aliased` per §3.2).
...
4. **`journal_entries` filter surface** — the category accepts LLM-supplied filter parameters (account, date range, description substring) rather than returning the whole ledger. Exact parameter shape (and any cap on date-range size, given large ranges could return a lot of rows into one prompt) needs pinning down at build time — flagged here so it isn't lost.
