...
- **P1-10 Command palette:** **SPEC ✅ 2026-07-22 — build after P1-3 remainder.** `:` in NORMAL mode opens a fuzzy command palette (center overlay, same family as the `?` overlay): page verbs DERIVED from the active FB.keys binding table (label = hint, executor = the binding's own `run`) + a small global registry (navigation + key-less API actions via fbApi with idempotency keys — no new backend surface). Rows show the key equivalent — palette doubles as a teacher. Mouse parity: `:` as first char in the topbar search opens it too; rows clickable. Deferred: arguments, aliases, chaining. Pairs with P2-6 (remap renames palette labels for free). Spec: payables-ux-spec.md §P1-10.
