...
**Status:** Shipped (2026-09-19 — top banner corrected to match the doc's own dated amendments below, which already record the four-tier cascade, §4.3/§4.4's FX-bounded matching, and the `toggleSettlement` mechanism as "Built same day"; README.md documents the cascade as current production behavior).
...
**Amended 2026-09-01 (§4 corrected — FX-bounded matching, corroboration-direction bug, candidate cardinality):** `fx_rounding` as originally written (§4.1) had no bound on discrepancy magnitude and only handled one direction of rate movement — found broken by direct code inspection, not a hypothetical. §4.1, §4.3, and a new §4.4 rewrite the foreign-currency path: no exact-match attempt for foreign bills (booking-vs-settlement rate drift makes bit-exact matches essentially impossible), a bounded expected-amount band anchored on the bill's own booking rate, a fixed direction bug in counterparty-name corroboration (checked the wrong substring direction — see §4.3), and a new candidate-cardinality confidence rule (a "match" against one of several plausible open bills is weaker evidence than a match against the only one). All still fully deterministic — no tier-4 involvement. **Follow-up same day:** the §4.3 corroboration-direction bug is confirmed still unfixed in code (`bank.js` L93/L141) — flagged as a hard prerequisite for §4.4, not an independent lower-priority item, since §4.4 makes name corroboration a gate. `FX_BAND_PCT` (§4.4) is specified as a company Setting (Settings → Extensions, alongside `vat_tolerance`), not a hardcoded constant. **Built same day:** §4.3's corroboration-direction fix (shared `corroborate()` helper in `bank.js`, bidirectional name match) and the full §4.4 mechanism (foreign bills excluded from exact-match and the generic tolerance loop, the booking-rate-anchored band, the corroboration gate, due-date proximity as a confidence modifier, and candidate-cardinality ambiguity handling) are implemented in `bank.js`, with `fx_match_band_pct` wired into `posting_rules.attr.list`/`.save` exactly as specified. Note the shipped `partial_payment` sub-classification inside §4.4 (an in-band but sub-50%-of-expected amount) and due-date-proximity numeric bands (±7 days / +0.05, >60 days / −0.10) were implementation-time choices not pinned down to specific numbers in the design pass — reasonable defaults in the same spirit as the other flat bands in §4.1, not independently reviewed. Covered by 6 new tests in `contract.test.js`, each using a dedicated fresh company to avoid interference from the hundreds of bills other tests in that file accumulate in `CT`. §4.2's 1:N cardinality search and §2.2/§2.3 of `bank-match-bill-settlement-spec.md` (multi-bill and FX settlement-on-approve) remain unbuilt — this amendment fixes matching, not the separate settlement-posting gap that spec covers.
**Amended 2026-09-01 (§4.4's toggle mechanism corrected — see `bank-match-bill-settlement-spec.md` §3.1):** the toggle-to-partial paragraph below originally proposed reusing `journal.propose`'s upsert path for a human reviewer to edit the settled amount — wrong, that path is actor-restricted to the original proposer. Built instead as a dedicated `bank.match.toggleSettlement` action, surfaced as a `~`-keyed Full/Partial toggle in Inbox (ratified: "a true/false toggle on Full/partial... no other click buttons"), plus a hard block in `journal.approve` (banner-surfaced) when required FX setup is missing. 2 new tests; full suite 185 green.
...
**2026-09-01: name-corroboration direction bug, found by inspection. FIXED same day** — a shared `corroborate(desc, partner, ref)` helper in `bank.js` now checks both directions, used by both the exact-match and tolerance loops (and §4.4's new FX path). Originally: the shipped substring check was `description.includes(vendorName)` — does the *bank description* contain the *full vendor name*. Real bank descriptions are almost always shorter/abbreviated versions of a vendor's registered name ("NORTHSTAR" on a statement vs. "NorthStar Pte Ltd" in master data), so a short string can never contain the longer one — this check failed on the common case, not just the edge case. Two fixes were specified, one shipped:
1. **Immediate (SHIPPED 2026-09-01):** check both directions — `partner.includes(desc) || desc.includes(partner)` — catches the common truncation case. This is what's live in `bank.js` today.
2. **Correct (not built):** stop growing a second, weaker string-matcher inside `bank.js` at all. §13 item 2 already specs a proper normalizer for exactly this problem (strip legal suffixes — AB, Inc, **Pte Ltd**, GmbH; strip bank-feed noise; trigram similarity; a curated alias table) for tier-3 master-data matching. Tier 2's corroboration should call that same normalizer rather than maintain its own ad hoc substring logic that drifts from it. `vendor_ref` (invoice/bill id) corroboration is unaffected by this bug — it already uses a word-boundary regex, not naive substring, and remains the stronger of the two signals.

**§4.4 shipped on top of the "immediate" fix, per the sequencing this section called for.** Both landed the same day, avoiding the partial-breakage scenario this note originally warned about. The "correct" fix (item 2, tier-3 normalizer reuse) remains a real follow-up — the bidirectional substring check is a real improvement over the original bug but is still a second, independent string-matcher from Tier 3's, and will still miss corroborations neither direction of substring can catch (e.g. genuine abbreviations like "NSTAR" rather than a clean prefix/suffix truncation).

### 4.4 Foreign-currency bills — bounded matching, not exact (added and built 2026-09-01)
...
`FX_BAND_PCT` is a flat percentage, in the same spirit as `early_payment_discount`'s flat 1–2% and `bank_fee_netted`'s flat 5–50, not a rate-history-derived or per-currency-pair value. A bank amount at or below `expected_max` (and, per §4.1's existing "amount ≤ invoice" conservatism, not below some sanity floor — an amount far too small to plausibly be this bill, e.g. under half of it, is a `partial_payment` candidate instead, not an FX-drift candidate) is eligible for the `fx_rounding` classification; anything larger falls through to tier 4, same as any other unmatched line.

**2026-09-01: `FX_BAND_PCT` is a company Setting, not a hardcoded constant.** Different companies transact in currency pairs with very different volatility — a flat default that's sensibly loose for one pair could be too tight for a genuinely volatile one, or too loose (letting wrong matches through) for a stable one. This is the same shape of knob `vat_tolerance`/`vat_tolerance_pct`/`bill_extraction_tolerance` already are, so it follows their exact wiring: a new key (`fx_match_band_pct`, suggested default 15%) added to `posting_rules.attr.list`/`posting_rules.attr.save` (`index.js`), rendering as one more row in the existing Settings → Extensions grid — no new UI surface, no new settings-storage mechanism. `matchLine` reads it the same way it already reads `default_bank_account` — a direct `SELECT value FROM settings WHERE key = '...'` query (`bank.js` ~L231) — not via the separate `companies` table lookup used for `accounting_method`/`currency`.
...
**Suggested-full, toggle-to-partial stays a human decision, not a matcher decision (revised 2026-09-01 — see `bank-match-bill-settlement-spec.md` §3.1).** Whether an FX-band shortfall represents "this bill is fully settled, the gap is rate drift" or "this is a genuine partial payment" is not something the matcher can resolve from the statement line alone — both produce the identical number. The proposal defaults to the classification's implied mode (`fx_rounding` → full, `partial_payment` → partial) but the human reviewer must be able to flip it explicitly, never have the system silently decide. **Not** via the propose-upsert-same-`proposalId` path originally sketched here — that path is actor-restricted to the original proposer (`created_by = @proposer`), so a human reviewer (a different actor from whatever proposed it) cannot use it. Built instead as a dedicated action, `bank.match.toggleSettlement`, surfaced in Inbox as a `~`-keyed true/false toggle on the row — the app's universal toggle verb, immediate-persist, no other click buttons. Separately, if the settlement math cannot be computed correctly at all (no valid `fx_rate` on the bill, or real drift with no FX Gain/Loss account configured), `journal.approve` refuses outright with a banner-surfaced error rather than falling back to an approximate entry or silently skipping settlement.
