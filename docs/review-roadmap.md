...
**K4 landed ✅ (this update):** attachment unification + reconcile verification. (A) **`A` = attach everywhere** — legacy `fbKeyActions` pages route shift-a to a new `attach` verb (common.js; bill-detail migrated off `a`); FB.form pages declare `A` (journal-voucher). (B) **journal-voucher pending queue** is an FB.form zone (`j`/`k` rows, `x` removes staged file; shared `.fb-attach-row` markup; new shared `api/public/fb-attachments.js` helper module). (C) **reconcile**: the audit's mouse-only-checkboxes item predates the FB.list migration — /bank/reconcile 301-redirects to /bank, whose Transactions tab already has j/k + `~` clear/unclear on the checkbox's own persistence path; closed with end-to-end verification (toggle + persistence), no new code. (D) **K3c regression fixed**: the default FB.form `active()` guard checked zone 0 only — journal-voucher (empty reversal zone at rest) and bank-import (closed bill panel at rest) were key-dead; guard now scans all zones. bill-edit queue nav deferred (K4b) *(resolved — PR #125, bill-edit migrated to FB.form, see §0hh)*; bill-detail keeps its bespoke combined nav (key unified only). Spec: keyboard-ux-spec §9 K4.
...
2. **Verb surface frozen.** No new keyboard verbs without explicit magnus ratification. `test:keys` gates ONE representative screen — **journal-voucher** (richest FB.form surface, primary human write path); all other routes get a load + zero-JS-errors smoke. Per-screen exemption tables retired (git history keeps them).
...
| **Journal UI** | Per-line computed-VAT readout cell; total VAT in totals bar; balance check includes VAT: `(dr + vatDebit) − (cr + vatCredit)`. | `api/src/pages/journal-voucher.js` |
...
**Bill-edit → FB.form migration ✅ (PR #125, closes #109).** The last bespoke form page — bill-edit — is migrated onto the shared `FB.form` framework. K4b (deferred since §0k) is resolved: bill-edit now uses the same zone/cursor/mode machine as journal-voucher, opening-balances, and bank-import. Every interactive page in the app now runs on `FB.form` or `FB.list`.
...
**Open issues (as of 2026-08-11, 15 open):** #110 (P3-1 AR), #114 (P2-7 coaStyle), #130 (fuzzy duplicate partner proposals), #131 (multi-bill settlement), #132 (bank-tab manual match), #133 (tolerance suggestions), #135 (palette frecency), #138 (reconciliation alert inbox item), #139 (PDF rasterization), #140 (prompt caching), #141 (FX automation admin page), #175 (simplify reference format), #179 (Settings/AI Action type), #180 (agent status home). Closed since §0hh: #108, #111, #112, #113, #116, #128, #129, #134, #136, #137, #149, #189 (duplicate PR).

---

## 0ii. Status update — 2026-08-11 (command-bar UX, IA restructure, opening-balance, Settings/AI, partners cleanup, bill-edit alignment)

**Command-bar UX overhaul ✅ (PRs #146–#160, #162–#165, #166–#174).** The `:` command palette and `/` search were rebuilt per the ratified `docs/command-bar-ux-spec.md`. Key changes:

- **Sidebar removed** (PR #146) — navigation lives in the command bar; typed grammar (`:show`, `:new`, `:report`) with scoped search; inbox fixes.
- **`:show` command** (PRs #162–#165) — replaces `:report`; palette browse mode for screen/tab/report navigation; period argument + filter by id; excludes action verbs; no parallel resolution system.
- **Command-bar v7** (PRs #167–#174) — prefix engagement (type `:s` → SHOW scope), flat list (no scope headers), ghost text via getComputedStyle, `:new` auto-activates add-entry, bare `:` = 6 inline aliases by workflow frequency with area labels, `:report` default period = latest posted txn, `:show` grouped by page, alphabetical tiebreaks, flat recent bonus. `:je` dropped.
- **Palette fixes** (PRs #147–#160) — label/route dedup, section grouping (PAGE/NAV/API), drop PAGE/NAV rows (issue #149, closed), dropdown persistence, alias visibility, page-verb removal, `_detectGrammar` trim bug, `:new` command + `:rate` date, alphabetical sort by label, frecency tiebreak, Periods page 404 fix.
- **Issues closed:** #134 (arguments/aliases/chaining — addressed by command-bar grammar), #136 (`/`-mode hits dropdown — superseded by scoped search), #149 (palette PAGE/NAV row cleanup).

**Opening-balance flattening ✅ (PRs #176–#181).** The opening-balance page was flattened per `docs/ia-restructure-spec.md`: reserved OPEN journal/period, moved inline under Settings tab, then converted to FB.list with column filters, then the tab was dropped entirely — opening balances now post via New JV (the Xero conversion-balances pattern inverted: one JV entry, no dedicated page). Issue: the old opening-balance page was the app's last orphaned standalone page.

**Settings/AI tab flattened ✅ (PRs #182–#183).** Three grouped form sections → single Attribute/Value/Type grid (mirrors Company tab). `ai.attr.list` (10 attributes) + `ai.attr.save` server actions. API keys use blank-keeps-stored convention. `agent_enabled` serialized as string boolean. `:show ai` command restored; number dirty-on-escape fixed. **Follow-up issues opened:** #179 (add `Action` type for Test connection row) and #180 (agent status needs a home — live operational state removed from settings grid, needs a dashboard or status-bar surface). PR #189 was a duplicate of #182 (same branch, created post-merge) — closed.

**Settings restructuring ✅ (PRs #184–#186, #191–#193).** Posting Rules split from Company tab (PR #185); FX Gain/Loss account moved to COA `default_role` column (PR #184); `:show` palette routes updated for the split (PR #186). **IA restructure** (PR #191) — Master Data / Settings / Admin split (per `docs/ia-restructure-spec.md`); `:show` routes fixed (PRs #192–#193). Settings/Company no longer shows as Settings/Settings.

**Partners migration completed ✅ (PRs #143–#145).** `vendor.*` deprecation aliases removed (issue #129, closed); `bills.vendor` column renamed to `partner_name` (issue #128, closed); all vendor references renamed to partner throughout the codebase. Issues #128, #129 closed.

**Bill-edit form alignment ✅ (PRs #187–#188, #190).** Bill-edit aligned with journal-voucher visual style; shared form CSS extracted to `common.css`; header inputs aligned; CR:AP account column aligned with DR:Expense column.

**Test harness cleanup ✅ (PR #127).** Self-contained tests wired into `npm test`; `playwright-core` added as devDep; CI installs root deps for `pdf-parse` (bill-extraction test); `mcp-smoke` skips gracefully when deps absent; `reversal.mjs` skips when browser binary missing (CI resilience).

**What stays (open backlog):** P2-7 coaStyle (#114) · P3-1 AR (#110) · #130 fuzzy duplicate partner proposals · #131 multi-bill settlement · #132 bank-tab manual match · #133 tolerance suggestions · #135 palette frecency · #138 reconciliation alert · #139 PDF rasterization · #140 prompt caching · #141 FX automation admin page · #175 simplify reference format · #179 Settings/AI Action type · #180 agent status home.
...
- **P1-6 Discoverability:** ✅ **DONE 2026-07-22** (branch `p1/discoverability`). `?` opens a which-key-style overlay of the active binding set — EXHAUSTIVE (every hinted binding, NORMAL | INSERT columns) where the sidebar stays the curated hintBar subset; both surfaces + dispatch generated from the same FB.keys table via shared `_groupHints` — cannot drift. NORMAL-mode-only trigger, never while typing; swallows keys while open; Esc/`?`/backdrop close; topbar `?` button wired as mouse parity. `:` palette **removed** (dispatched to undefined `fbCmdDispatch`; `fbOpenCmdPalette` a no-op stub — doubly dead code, deleted per rule 6; proper replacement specced as P1-10). Cache-buster bumped. Suite 28/28. **Overlay restructured (PRs #283–#286)** to NAVIGATION + ACTIONS two-column layout (per `docs/help-overlay-chrome-spec.md` v3): INSERT column removed; NAVIGATION = static chrome (`/ : h/l j/k gg/G` + g-map "Go to " rows); ACTIONS = page NORMAL bindings under the page/tab name (not "ACTIONS"); `j`/`k`/`h`/`l` deduped; hints capitalized; Esc/`?`/standalone-`g` omitted. **Key consolidation (PRs #287–#288):** `_keyLabel` Ctrl/Cmd probe removed (dead code); Space relabeled "Expand/Collapse"; Enter relabeled "edit" on FB.list; bill-detail `e`→`i` (edit), `d`→`x` (delete/void); `~` silent alias for Space on tree pages (not shown in overlay). **ACTIONS column fixes (PR #293, per `docs/help-overlay-chrome-spec.md` v4):** three gaps between the ratified spec and the actual code, caught by user review — heading was the raw `FB.keys` registration id (e.g. `md-coa`) instead of a curated page/tab name for every tab whose `keysId` wasn't already a clean word; `_groupHints` never merged same-key/different-hint bindings (Bills' `x` = void or delete showed as two rows) even though the spec's own example already said "Delete/void"; row order was raw dispatch-precedence order (page-specific verbs first) rather than commonality order. Fixed by adding an optional `heading`/`label` field read ahead of the `_cap(keysId)` fallback, a same-key merge pass in `_groupHints` (shared with the sidebar hint bar), and a commonality-tier sort scoped to the overlay only. **Further consolidation (PR #293, same review):** `q` quit → `Escape` on `bill-edit`/`journal-voucher` (Escape is the universal cancel/back key already; `q` was redundant — deliberately un-hinted, no sidebar/overlay row needed); `journal-voucher`'s quit gained the dirty-content confirm guard it was missing (bill-edit already had one, JV silently discarded); JV reversal moved off `R` onto `u` (reuses the universal undo key — a posted batch is immutable, so a reversing entry IS the undo-equivalent) and the draft-compose reversal-search flow (search any batch, apply its lines into a fresh entry) was removed entirely — reversal is now reachable only from a loaded posted batch (`?batch=`). `tests/reversal.mjs` rewritten for the new entry point; `tests/keys-coverage.mjs` exemptions updated (`q`→`Escape`, `R`→`u`). **i/a/Enter consolidation (PR #293, same review, app-wide):** three exclusive roles across every FB.list/FB.form page, per `docs/keyboard-ux-spec.md` §8 — `Enter` = edit exclusively (no-op change, already true everywhere: FB.form's `i` and `Enter` were literal duplicates, FB.list's `Enter` already covered everything `i` did); `a` = attach exclusively, moved off Shift-`A` on `bill-detail`/`bill-edit`/`journal-voucher` (the now-dead legacy `common.js` bubble-phase `A` handler deleted — FB.keys pages already shadowed it); `i` = insert exclusively — FB.form's built-in `i`→edit binding deleted, each page's `verbs.add` rekeyed `a`→`i`. On FB.list, `i` now calls `newRow()` **from anywhere**, not just when the "+ Add entry" row happens to be focused — closes a real inconsistency where Bills had a direct add key (`a`) but every flat grid (Tax Codes, Settings, Access, …) required navigating to the add row first. Bills' existing "add a line to the focused draft bill" binding moved from `a` to `i` unchanged (more specific, wins via first-match when a draft bill is focused; falls through to the generic new-row fallback otherwise — same key/hint text, so `_groupHints` collapses them to one overlay row). Guarded on `canAdd` (silent on fixed-row registers). `bill-detail` has no add-line concept, so `i` is simply unbound there now. `docs/keyboard-ux-spec.md` and `docs/payables-ux-spec.md` updated for the pages actually touched; both docs still carry pre-existing stale sections predating this session (payables-ux-spec's old Partners/Vendors INSERT-mode writeup predates the FB.list migration; its bill-editor "design proposal" section predates the shipped `I` key) — left alone, out of scope. **Bugfix, same review:** the tree-specific `i` binding was gated only on `cfg.tree`, not on whether the page can actually add a line — Inbox and the Journal register are also `tree: true` (for fold/expand) but define no `cfg.addChild` and hardcode `editable() → false`, so they inherited the "insert line item" hint in the `?` overlay (which never evaluates `when`) despite having nothing for it to do. Gated on `cfg.addChild` directly; only Bills defines it, so only Bills advertises insert now. **Enter/fold consolidation (PR #293, same review, app-wide):** Space is now the only key that folds, on every tree page — `openFocused()`'s tree branch used to also fold a read-only row (posted bill, proposal), duplicating Space, which already toggles fold independently off `focusedRow()` regardless of Enter. Removed; a non-editable tree row is now a clean no-op for Enter, same as a non-editable flat row already was. This freed Enter on Inbox's bill-due rows (not editable, previously a no-op-via-fold) for something real: the `o` rowVerb ("open bill" → Payables) moved to `Enter` — no separate chord needed since Enter had nothing else to do there. `docs/fb-list-ux-spec.md` §4/§5/§6 updated (the `a`→`i` add-child rename from the i/a/Enter consolidation above hadn't been reflected there yet either — fixed in the same pass). **Kill human draft-save on Bills (PR #293, same review):** a human's only path to persisting a bill is now posting — `w` (Bills grid, and `bill-edit.js`'s full editor) now runs the same `postDraft`/`postBill` path as `p` (silent alias on the editor; merged into one "w/p Post/pay" row on the grid) instead of `bill.draft.save`. Agent-created drafts are untouched (`bill.create` is `agentWritable`; the handler still forces `status='draft'` for an agent actor, reviewed via Inbox exactly as before) — a human opening one of those and pressing `w`/`p` posts it, since `postDraft`/`postBill` already silently persist-then-post as one gesture when needed. New `cfg.draftSaveOnLeave: false` (`fb-list-ux-spec.md` §6) makes the shared leave-guard modal drop "Save" too, set on Bills. **Known gap, not closed this round:** the generic per-row ✓ "write" mouse chip (`fb-list.js`, renders for any dirty row app-wide) still calls plain `writeAt`/`bill.draft.save` directly — it's the one mouse-accessible path left that can still park a bill in draft. Closing it needs a `cfg` hook the chip's click handler can check, analogous to `draftSaveOnLeave`. **Inbox approve, `y`→`w` (PR #293, same review):** the user's own cross-page shortcut audit caught this — approve is the same "commit this to the ledger" action `w` already means everywhere else (write on Chart of Accounts, post on Bills/Journal Voucher), so it joins that family instead of a bespoke `y`. Reject stays on `x`, already consistent with the app-wide delete/void/reject convention. `w` was never otherwise reachable on Inbox rows (never dirty — no in-place edit — so the built-in FB.list `w` binding, gated on a dirty row, never matched there), so no collision.
- **P1-10 Command palette:** ✅ **DONE 2026-07-23** (initial); **rebuilt 2026-08-09–11** as the command-bar UX (PRs #146–#174). Initial version: topbar input with `:` = command mode, `/` = search, hits dropdown, page verbs + catalog API actions via disposition map. **Command-bar rework** (per `docs/command-bar-ux-spec.md`): sidebar removed; typed grammar (`:show`, `:new`, `:report`); `:show` replaces `:report` (palette browse mode for screen/tab/report navigation); prefix engagement (`:s` → SHOW scope); flat list (no scope headers); ghost text via getComputedStyle; bare `:` = 6 inline aliases by workflow frequency with area labels; `:new` auto-activates add-entry; `:report` default period = latest posted txn; `:show` grouped by page; alphabetical sort + frecency tiebreak. `:je` dropped. Issues #134, #136, #149 closed. See §0ii.
