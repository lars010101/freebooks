...
**Date:** 2026-08-27 · **Status:** Shipped (verified against code 2026-09-19) — `nav-registry.js`'s own header comments cite "2026-08-27 IA restructure 2" matching commit `9303d05`; current `nav-registry.js` routes are exactly `payables, bank, statements, journal, calendar, documents, settings, accounting, exchange-rates` — no `bills`/`master-data`/`admin`/`reports`/`periods` entries, matching this spec's design.
...
| Date | Change |
|------|--------|
| 2026-08-27 | Spec drafted per design-review discussion: Payables (Bills+Vendors+Aging+Control) replaces Master Data's Partners split and pulls AP reports out of Reports; Reports split into Statements + Reports; Periods renamed Filings and fully flattened (tree removed); Settings split into Settings + Accounting; Master Data dissolved (Exchange Rates promoted standalone); Admin dissolved (Companies → switcher, Access → Settings, Operations dropped); VAT/GST Tolerance moved to command-bar-only, no UI surface; per-page `dateRelevance` flag stubbed for a follow-up chrome spec. Status: PROPOSED, pending ratification (**superseded 2026-09-19 — see below, this restructure shipped**). |
| 2026-09-19 | Doc reconciled against code: status flipped from PROPOSED to Shipped. `nav-registry.js`'s route table confirmed to match this spec's design exactly (payables/bank/statements/journal/calendar/documents/settings/accounting/exchange-rates, no bills/master-data/admin/reports/periods). |
