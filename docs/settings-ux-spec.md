...
1. **Tab content restructure** (lands with each tab's own migration):
   ...
   - **VAT Tolerance** panel → ~~moves into the Tax Codes tab~~ **superseded 2026-07-27 (rev. 3):** becomes two typed rows on the Posting Rules grid (rev. 4, 2026-08-26 — moved from Company grid).

   **Rev. 2026-07-27 (rev. 3, ratified in thread):** the slim record form is itself replaced by an **FB.list attribute/value grid** — the Company tab is no longer the odd non-list surface. Fixed rows (no add, no delete — every attribute is critical), columns `Attribute | Value | Type`; only the Value cell edits (`i`/Enter/click → typed editor per row: text / number / checkbox / choice select); `w` writes ONE attribute per dirty row via `company.attr.save`; `u` reverts; Esc never saves. The Type column is informational (String/Number/Boolean/Choice); client editors constrain input shape only — **validation is server-authoritative** (`company.attr.save` checks per attribute on write). Company ID is a read-only row (`editable: false`). **Rev. 2026-08-26 (rev. 4, ratified in thread):** Multi-Currency, FX Provider, FX API Key, and the two VAT Tolerance rows **moved off the Company grid** onto a separate **Posting Rules tab** with its own attribute grid (same FB.list pattern: `Attribute | Value | Type`, per-row edit/commit). The Company grid retains entity-only attributes (name, currency, jurisdiction, tax_id, reporting_standard, vat_registered, contact attributes). The Posting Rules grid persists via `posting_rules.attr.list` / `posting_rules.attr.save` — a different action namespace than `company.attr.*`. This split keeps entity identity separate from posting-behavior configuration. **✅ SHIPPED 2026-08-26:** all five rows live on the Posting Rules tab; Company grid no longer carries FX or tolerance attributes.
