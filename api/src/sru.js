...
// ── INFO.SRU ────────────────────────────────────────────────────────────────
function buildInfoText(company, params, contact = {}) {
...
async function handleSruInk2(req, res) {
  ...
  try {
    ...
    // computeSru so the short-circuit fires even on empty books — Skatteverket
    // rejects INFO.SRU with blank #POSTNR/#POSTORT regardless of the blanket
    // contents. With ?check=1 the problems are appended to warnings and never
    // block the check flow.
...
async function handleSruInfo(req, res) {
  ...
  try {
    ...
    const text = buildInfoText(co, { kontakt, telefon, email }, contact);
    res.setHeader('Content-Type', 'text/plain; charset=utf-8');
