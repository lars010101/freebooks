...
**Status:** IMPLEMENTED, WITH TWO CONFIRMED GAPS — revised 2026-08-26 after a second validation pass against live code (`agent-loop.js`, `journal.js`). All four functions (`processJournalDocument`, `extractJournalDocumentData`, `buildJournalExtractionContext`, `buildJournalExtractionPrompt`) are live, and the core extraction logic — net computation, balanced-line construction, cross-checks, the `Cash` `default_role` lookup, duplicate detection, unconditional triggering (§1.1) — is all confirmed correctly implemented. Two gaps remain, both in §4.1: the `proposalId` is a freshly-minted uuid rather than the attachment's own `entity_id`, breaking underlag lookup; and `_extraction_meta` is computed but never reaches `journal_proposals` because `journal.propose` only reads `_match_meta`. Both are described with their exact fix below.
**Depends on:** `agent-data-feeding-guide.md` §4.3 (folder→`entityType` routing), §3 (A4 underlag binding — §4.1 documents a confirmed bug in how this spec's own implementation currently violates that binding), §5 ("what good agent output looks like" — the balanced-lines target shape this spec produces); `bill-extraction-spec.md` (sibling spec — shares §2.1's document-read logic by reference, diverges on output shape and account model); `bank-matching-spec.md` §1 (the `lines[]` shape this spec's output matches), §8.3 (agent orchestration precedent); `p2-4a-vat-unify-spec.md` (journal-entry postings are tax-**exclusive** — the single most important constraint this spec has to get right, §3.3); `agent-readiness-spec.md` R2/R5/R7/R8, §4 (single-gateway rule), §10.2 (Class A taxonomy).
...
**Correct behavior:** `processJournalDocument()` must call `journal.propose` using `ev.entity_id` — the attachment's existing id, established when the drop-folder watcher uploaded it — as the `proposalId`, not a freshly minted one. This is what makes the A4 underlag sequence (`agent-data-feeding-guide.md` §3) actually work: `attachmentCountForProposal()` (`journal.js`) looks up underlag by `WHERE entity_id = @proposalId`, so the proposal's id and the attachment's `entity_id` have to be the *same* value for that lookup to find anything.

**Confirmed bug, second validation pass:** as deployed, `processJournalDocument()` mints a genuinely fresh UUID instead of reusing `ev.entity_id` (`agent-loop.js` line 1904). This breaks the lookup above — the attachment is still there, but filed under a different id than the one the proposal now carries, so `attachmentCountForProposal()` finds nothing and the proposal gets a spurious `no_underlag` warning even though the source document is genuinely attached. An earlier draft of this section itself contained the same confusion, asserting "the entityId *is* the proposalId" as if that were already guaranteed by construction — it isn't; it has to be done deliberately, on purpose, in this function, and as deployed it currently isn't. **Fix: replace the freshly-minted uuid with `ev.entity_id` directly.**

**Second confirmed gap, same pass: extraction metadata is silently dropped.** `processJournalDocument()` passes `_extraction_meta` (model, confidence, flags, raw output, prompt/context snapshot) when calling `journal.propose` (`agent-loop.js` line 1911) — but `journal.propose` (`journal.js` line 944) only destructures `_match_meta` from its input, not `_extraction_meta`. The audit data this spec relies on for reproducibility (§ elsewhere in this document) is computed and then thrown away. **Fix: send it as `_match_meta` instead of `_extraction_meta`** — simpler than extending `journal.propose`'s parameter surface for a second, functionally-identical metadata field. `_match_meta` already exists precisely to carry "how did the agent decide this" for a proposal, regardless of whether the proposal came from the bank-matching cascade or from document extraction; there's no reason the two paths need differently-named payloads for the same purpose.
...
| File | Change |
|---|---|
| `api/src/agent-loop.js` | ~~Add `processJournalDocument()`, etc.~~ — **done**, all four functions confirmed live. **Remaining fix:** line 1904 — replace the freshly-minted uuid with `ev.entity_id` when calling `journal.propose`, so the proposal's id matches the attachment's existing `entity_id` (§4.1). |
| `api/src/journal.js` | **Remaining fix:** line 944 — `journal.propose` only destructures `_match_meta`; `processJournalDocument()` sends `_extraction_meta` under a different key, so it's silently dropped. Either accept `_extraction_meta` as an alias, or (simpler) have `processJournalDocument()` send it as `_match_meta` (§4.1). |
| `api/src/pages/master-data.js` | ~~Add `'Cash'` dropdown choice~~ — **done**, confirmed at line 402. |
| `api/src/index.js` / `coa.upsert` handler | ~~Extend single-holder enforcement~~ — **done**, confirmed at lines 491, 506–513. |
| `db/schema.sql` | No change — `accounts.default_role` already exists as a free-text column |

## 10. Open questions

1. **Duplicate-detection window** — §6 proposes same-day ±1 as a starting point; untested against real receipt-drop patterns (e.g. a recurring daily parking charge would need a tighter or amount-plus-description-specific window to avoid false-positive duplicate flags). Confirmed implemented as specified (`_checkJournalDuplicate()`, ±1 day + 0.70 similarity) — this question is about tuning the threshold, not whether it exists.
