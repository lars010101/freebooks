...
**Open by priority:** P1-1 action catalog/schemas (unlocks MCP, generates contract tests) → P1-8 read models → P1-4 bill editor rebuild (biggest visible UX win; kills the third entry UI) → P1-6 "?" overlay → P1-3 remainder (journal/bank/settings onto FB.keys) → P1-10 `:` command palette (spec ✅ 2026-07-22; build after P1-3 so derived page verbs cover the whole app) → P2 accounting completeness (year-end close, FX reval monetary-only, `bill_lines` subledger, server-computed draft totals, VAT convention unify) → P3 scope (AR, feeds).
...
- **P1-3 Shared UI core:** one mode manager, one key dispatcher, one nav abstraction, one utils module (`esc`, `fmtDate`, `statusBadge`, autocomplete). Migrate Bills onto it; then Vendors, Journal, Bank, Settings follow the payables pattern. Eliminate the `_fbVimMode`/`cursor._mode` duality. **Status 2026-07-22:** fb-core exists (FB.mode/FB.keys/FB.nav/FB.util); Bills ✅, Vendors ✅, **Bank ✅** (Transactions tab: j/k cursor via FB.nav, `c` clear/unclear, Esc semantics, hints follow tab switch; deleted ~714 LOC of dead legacy-import code that lived in bank.js + fixed two toggleCleared bugs — filtered-index corruption + focus wipe; Mappings tab deferred — per-row-input table, not a vim list). Remaining: Journal (dashboard journal view + journal-new), Settings.
...
- **P1-10 Command palette:** **SPEC ✅ 2026-07-22 (magnus's design) — build after P1-3 remainder.** The existing topbar input hosts two modes: `/` = search (unchanged), `:` = command mode with a hits dropdown below the input — NO separate overlay/bar (magnus decision 1). Mouse entry is always search; commands are keyboard-only (decision 2). Sources: page verbs derived from the active FB.keys binding table (executor = the binding's own `run`) + ALL catalog API actions via an execute/navigate-to-form/excluded disposition map (decision 3 — new actions default to navigate, palette grows with the API). Rows show key equivalents — palette doubles as a teacher. Deferred: arguments, aliases, chaining, `/`-mode hits dropdown. Pairs with P2-6. Spec: payables-ux-spec.md §P1-10.
