...
**Date:** 2026-08-18 · **Status:** Shipped (verified against code 2026-09-19) — `journal-voucher.js` has `entry-ccy` (fxTracking-gated), `.cc-input`/`.pc-input` (centersConfigured-gated), and the mutual-exclusivity wiring described here. · **Scope:** `/:company/journal/voucher` only (header + line fields, read-only/view-mode rendering, reversal rendering). No other FB.form screen is in scope.
...
| 2026-09-19 | Doc reconciled against code: status flipped from PROPOSED to Shipped. Confirmed `journal-voucher.js` carries `entry-ccy`, `.cc-input`/`.pc-input`, and the mutual-exclusivity wiring described above. |
