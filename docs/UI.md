...
- **Tabs, one `FB.list` instance per tab.** `.tabs`/`.tab`/`.tab.active`/`.tab-panel`/`.tab-panel.active` (page-local CSS recipe — not yet promoted to common.css, copy it verbatim per page: accounting.js/settings.js/payables-bills.js+payables-partners.js/Inbox all do). FB.list's columns are fixed per instance (see the conditional-column bullet above), so a tab strip whose tabs show genuinely different columns needs one `FB.list.create()` per tab, not one instance with a swapped column config — there is no such swap API. Each instance's `active()` callback checks its own `.tab-panel.active` class so only the visible tab's keybindings are live; call `list.renderHints(hintEl)` (not the global `FB.keys.renderHints(...)` directly) on tab switch — it already knows its own `keysId`. Load every tab's list eagerly at page load only if the tab strip itself needs to show live per-tab counts (Inbox); otherwise lazy-load on first visit and cache (accounting.js/settings.js's `tabLoaded[t]` guard).
