...
**Date:** 2026-08-29 · **Status:** Shipped (verified against code 2026-09-19) — `api/src/pages/calendar.js`, `documents.js`, `reminder-scanner.js`, `attachment-integrity-scanner.js`, and `orphaned-files.js` all exist and the `reminders`/`orphaned_files` tables are live in `db/schema.sql`, matching this spec.
...
**Resolution happens on Documents, not Inbox or the notification dropdown (2026-09-08, superseding the original placement below).** The bell stays purely a ping (as it is today — `fb-core.js:3088-3157` has no per-item actions beyond mark-read). Originally this spec placed resolution in Inbox, reasoning that an orphaned file needing a human verdict was the same shape of task as an accept/reject review. In practice that framing didn't hold: there's no proposal to accept, no data to approve — the only decision is "keep on disk or delete," which is a file-registry housekeeping task, not a review-queue approval. Documents is already the file registry (§5.1); an orphan row there (`entity_type:'orphan'`, `api/src/pages/documents.js`) has no `attachment_id`/period/source (there's no DB row to have one), and gets two actions instead of the usual Open/Go-to-source/Delete:
  - **Open** — direct-serves the file (`GET /api/orphaned-file/:orphanId`, `orphaned-files.js`'s `serveOrphanFile`), since there's no `attachment_id` to serve it by.
  - **Delete** — `orphan.delete` deletes the blob directly off disk (there's no `attachment.delete` to call; no attachments row exists). No app-managed quarantine — the operator downloads a copy via Open first if one is wanted.

  The read side is `orphan.list` (`orphaned-files.js`), viewer/non-mutating, filtered to `resolved_at IS NULL` — moved out of `inbox.js`'s `queryOrphanedFiles` (the original placement) since Inbox no longer surfaces this item kind at all.
...
5. **Orphaned-file schema and semantics** (§5.5) — the `orphaned_files` row shape is now sketched and resolution lives on Documents (View/Delete only — no Move was built), but how a hand-placed file outside the `${companyId}/${entityType}/${entityId}/...` convention (§5.5) — one with no company to attribute to at all — should surface is still open, given Documents review is inherently company-scoped.
...
| 2026-09-08 | §5.5 reversed: orphaned-file resolution moved from Inbox to Documents. In practice there was no proposal to accept/reject — the only decision is keep-or-delete, a file-registry housekeeping task, not a review-queue approval. `inbox.js`'s `queryOrphanedFiles`/`status:'orphans'` removed; a new `orphan.list` viewer action (`orphaned-files.js`) backs a Documents `entity_type:'orphan'` row (Open serves the file directly, Delete calls `orphan.delete`, same as before). Inbox's `f` filter cycle drops to five states (proposed/rejected/partners/suggestions/rejections). §9 open question 5 updated to match. |
| 2026-09-19 | Doc reconciled against code: status flipped from PROPOSED to Shipped. All consumer files listed in the header (`calendar.js`, `documents.js`, `reminder-scanner.js`, `attachment-integrity-scanner.js`, `orphaned-files.js`) confirmed present; `reminders` and `orphaned_files` tables confirmed in `db/schema.sql`. |
