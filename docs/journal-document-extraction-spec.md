...
**Status:** IMPLEMENTED — both previously-open gaps confirmed fixed 2026-09-19 on a third validation pass against live code (`agent-loop.js`, `journal.js`). All four functions (`processJournalDocument`, `extractJournalDocumentData`, `buildJournalExtractionContext`, `buildJournalExtractionPrompt`) are live, and the core extraction logic — net computation, balanced-line construction, cross-checks, the `Cash` `default_role` lookup, duplicate detection, unconditional triggering (§1.1) — is all confirmed correctly implemented. The two gaps §4.1 used to describe (fresh-uuid `proposalId` instead of `ev.entity_id`; `_extraction_meta` silently dropped by `journal.propose`) are both resolved in the current code — see §4.1 and §9 for the as-built citations.
...
**Fixed, confirmed 2026-09-19:** `processJournalDocument()` (`api/src/agent-loop.js:2026`) now reads `const proposalId = ev.entity_id;` — reusing the attachment's existing id exactly as this section specifies, so `attachmentCountForProposal()`'s underlag lookup finds it. (Historical note: an earlier deployed version minted a fresh UUID here instead, which broke the lookup — that has since been corrected.)

**Second gap, also fixed, confirmed 2026-09-19: extraction metadata now reaches `journal_proposals`.** `processJournalDocument()` (`api/src/agent-loop.js:2033`) now passes the extraction's model/confidence/flags/raw-output payload as `_match_meta` when calling `journal.propose`, matching the key `journal.propose` (`api/src/journal.js:1012`) actually destructures from its input. (Historical note: an earlier deployed version sent this payload under the key `_extraction_meta`, which `journal.propose` didn't read, so it was silently dropped — that has since been corrected by sending it as `_match_meta` instead, the same convention `processBill()`'s bank-matching path already uses.)
...
## 9. Files changed (confirmed against source, third pass, 2026-09-19)

| File | Change |
|---|---|
| `api/src/agent-loop.js` | ~~Add `processJournalDocument()`, etc.~~ — **done**, all four functions confirmed live. ~~Remaining fix: reuse `ev.entity_id`~~ — **done**, confirmed at line 2026 (§4.1). ~~Remaining fix: send `_match_meta` not `_extraction_meta`~~ — **done**, confirmed at line 2033 (§4.1). |
| `api/src/journal.js` | `journal.propose` destructures `_match_meta` at line 1012 — confirmed this is now the key `processJournalDocument()` actually sends (§4.1). |
